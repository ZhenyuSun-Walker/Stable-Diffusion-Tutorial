...
class DDPMSampler:
    ...
    def __init__(self, generator: torch.Generator, num_training_steps=1000, beta_start: float = 0.00085, beta_end: float = 0.0120):
        ...
        self.alpha_cumprod = torch.cumpord(self.alphas, 0)
        self.one = torch.tensor(1.0)

        self.generator = generator
        self.num_training_steps = num_training_steps
        self.timesteps = torch.form_numpy(np.arrange(0, num_training_steps)[::-1].copy())
    ...
    def _get_variance(self, timestep: int) -> torch.Tensor:
        ...
        # Compute according to the formula (7) in DDPM paper
    ...
    def step(self, timestep: int, latents: torch.Tensor, model_output: torch,Tensor):
        ...
        alpha_prod_t = self.alpha_cumprod[timestep] 
        alpha_prod_t_prev = self.alpha_cumprod[prev_t] if t >= 0 else self.one
        beta_prod_t = 1 - alpha_prod_t
        beta_prod_t_prev = 1 - alpha_prod_t_prev
        current_alpha_t = alpha_prod_t / alpha_prod_t_prev
        current_beta_t = 1 - current_alpha_t

        # Compute the predicted original sample using formula (15) in DDPM paper
    ...
    def add_noise(self, original_samples: torch.FloatTensor, timesteps: torch.IntTensor) -> torch.FloatTensor:
        ...
        # According to the equation (4) from the DDPM paper
